# WiFi Remote Desktop Client - Connects to server over WiFi network
...
class RemoteClientWiFi:
    def __init__(self, server_ip, port=8888):
        self.host = server_ip               # Server IP address from command line
    ...
    def connect_to_server(self):
        """Connect to the remote server over WiFi"""
        # Create a TCP socket
        ...
        try:
            ...
            print(f"Make sure:")
            print(f"  1. Server is running on {self.host}")
            print(f"  2. Both computers are on same WiFi network")
            print(f"  3. Windows Firewall allows the connection")
            return False
...
def print_usage():
    ...
    print("Usage: python client_wifi.py <server_ip_address>")
    print("Example: python client_wifi.py 192.168.1.100")
    print("\nTo find the server's IP address:")
    print("Run 'python find_ip.py' on the server computer")

if __name__ == "__main__":
    # Check if IP address was provided as command line argument
    if len(sys.argv) != 2:
    ...
    if not server_ip.replace('.', '').replace(':', '').isdigit():
        print(f"Error: '{server_ip}' doesn't look like a valid IP address")
        print_usage()
        sys.exit(1)
    
    # Create and start the WiFi client
    client = RemoteClientWiFi(server_ip)
    try:
        client.start_client()
